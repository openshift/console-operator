...
func (c *ManagedClusterController) SyncManagedClusterConfigMap(managedClusters []clusterv1.ManagedCluster, ctx context.Context, operatorConfig *operatorv1.Console, recorder events.Recorder) (string, error) {
	...
	for _, managedCluster := range managedClusters {
		...
		if apierrors.IsNotFound(err) {
			klog.V(4).Infof("API server CA file not found for managed cluster %q", clusterName)
			continue
		}

		// Skip if unable to get managed cluster API server config map for any other reason
		if err != nil {
			klog.V(4).Infof("Error getting API server CA file for managed cluster %q", clusterName)
			continue
		}
		...
		if apierrors.IsNotFound(err) {
			klog.V(4).Infof("OAuth server CA file not found for managed cluster %q", clusterName)
			continue
		}

		// Skip if unable to get managed cluster OAuth server config map for any other reason
		if err != nil {
			klog.V(4).Infof("Error getting OAuth server CA file for managed cluster %q", clusterName)
			continue
		}
		...
		if apierrors.IsNotFound(err) {
			klog.V(4).Infof("OAuthClient ManifestWork not found for managed cluster %q", clusterName)
			continue
		}

		// Skip if unable to get managed cluster OAuth client ManifestWork for any other reason
		if err != nil {
			klog.V(4).Infof("Error getting OAuthClient ManifestWork for managed cluster %q", clusterName)
			continue
		}

		if !meta.IsStatusConditionPresentAndEqual(oAuthClientWork.Status.Conditions, workv1.WorkAvailable, metav1.ConditionTrue) {
			klog.V(4).Infof("OAuthClient is not yet available on managed cluster %q", clusterName)
			continue
		}

		oAuthClientSecretString, err := manifestworksub.GetOAuthClientSecret(oAuthClientWork)
		if err != nil {
			klog.V(4).Infof("Error getting OAuthClient secret string for managed cluster %q", clusterName)
			continue
		}
		...
		if apierrors.IsNotFound(err) {
			klog.V(4).Infof("OLM config ManagedClusterView not found for managed cluster %q", clusterName)
			continue
		}
