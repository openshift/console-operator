...
func (co *consoleOperator) SyncCustomLogoConfigMaps(operatorConfig *operatorv1.Console) ([]configmapsub.CustomLogoRef, error, string) {
	configMapsToSync := getCustomLogoConfigMapRefs(operatorConfig)
	var (
	...
	for _, logoToSync := range configMapsToSync {
		err, reason = co.syncCustomLogoConfigMap(logoToSync)
		if err != nil {
...
func (co *consoleOperator) syncCustomLogoConfigMap(customLogo configmapsub.CustomLogoRef) (error, string) {
	// validate first, to avoid a broken volume mount & a crashlooping console
	err, reason := co.validateCustomLogo(customLogo.File)
	if err != nil {
		return err, reason
	}
...
func (c *consoleOperator) updateCustomLogoSyncSource(cmRef operatorv1.ConfigMapFileReference) error {
