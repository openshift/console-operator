...
var map_ConsolePluginProxy = map[string]string{
	...
	"services": "services is a list of in-cluster Services that the plugin will connect to. The Service must use HTTPS. Console backend exposes following endpoint in order to proxy the communication between plugin and the Service:\n\n/api/proxy/namespace/<service-namespace>/service/<service-name>:<port-number>/<request-path>?<optional-query-parameters>\n\nRequest example path:\n\n/api/proxy/namespace/helm/service/helm-charts:8443/releases?limit=10",
}
...
var map_ConsolePluginSpec = map[string]string{
	...
	"displayName": "displayName is the display name of the plugin. displayName is the display name of the plugin.",
